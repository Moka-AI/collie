...
class GLU(torch.nn.Module):
    def __init__(
        self, hidden_size, inner_hidden_size=None, layer_id=None, bias=True, activation_func=gelu, params_dtype=torch.float
    ):
        ...
        if past_key_values is None:
            ...
            if position_ids is None:
                ...
                position_ids = self.get_position_ids(
                    input_ids, mask_positions=mask_positions, device=input_ids.device, gmask=use_gmask
                )
    ...
    def _update_model_kwargs_for_generation(
        ...
        if 'attention_mask' in model_kwargs:
            ...
            if attention_mask is not None and attention_mask.dtype == torch.bool:
                attention_mask = torch.cat([attention_mask, attention_mask.new_ones((*attention_mask.shape[:3], 1))], dim=3)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past is not None or past_key_values is not None:
            ...
            if position_ids is None:
                position_ids = self.get_position_ids(
                    input_ids, device=input_ids.device, mask_positions=mask_positions, gmask=use_gmask
                )
